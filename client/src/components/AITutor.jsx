...
export function AITutor() {
    ...
    var handleSend = function () {
        ...
        setTimeout(function () {
            var randomResponse = aiResponses[Math.floor(Math.random() * aiResponses.length)];
            var aiMessage = {
    ...
    return (<div className="glass-card rounded-[2rem] border-white/20 overflow-hidden flex flex-col shadow-xl h-[600px] bg-white/50 dark:bg-black/50 backdrop-blur-xl">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
        {messages.map(function (message) { return (<div key={message.id} className={cn("flex w-full", message.isUser ? "justify-end" : "justify-start")}>
            <div className={cn("max-w-[85%] rounded-[1.5rem] p-5 shadow-sm transition-all animate-fade-in-up", message.isUser
              ...
              <div className="flex items-start gap-3">
                <div className="flex-1 space-y-1">
                  ...
                  {message.hindiText && (<p className={cn("text-sm", message.isUser ? "text-primary-foreground/70" : "text-muted-foreground")}>
                      {message.hindiText}
                    </p>)}
                </div>
                {!message.isUser && (<button onClick={function () { return speakText(message.text); }} className="p-2 hover:bg-black/5 rounded-full transition-colors flex-shrink-0">
      ...
      <div className="p-4 bg-white/50 dark:bg-black/20 backdrop-blur-md border-t border-white/10">
        <div className="flex items-end gap-3 bg-white dark:bg-slate-900 p-2 rounded-[1.5rem] shadow-lg border border-border/50">
          <textarea value={input} onChange={function (e) { return setInput(e.target.value); }} placeholder="Ask Arya anything..." className="flex-1 min-h-[50px] max-h-[120px] p-3 bg-transparent border-0 focus:ring-0 resize-none text-base placeholder:text-muted-foreground/70" onKeyDown={function (e) {
          ...
          <div className="flex gap-2 pb-1 pr-1">
            <button onClick={function () { return alert("Coming Soon!"); }} className="p-3 rounded-xl hover:bg-secondary text-muted-foreground hover:text-foreground transition-all">
              <Mic className="h-5 w-5"/>
            </button>
            <button onClick={handleSend} disabled={!input.trim()} className="p-3 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
