...
export function AdvancedGrammarCoach() {
  ...
  const [selectedSample, setSelectedSample] = useState<keyof typeof sampleTexts>('beginner');
  ...
  const loadSample = (sample: keyof typeof sampleTexts) => {
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="rules" className="space-y-4">
              <div className="grid gap-4">
                {Object.entries(grammarRules).map(([key, rule]) => (
                  <Card key={key}>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
