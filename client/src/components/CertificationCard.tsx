import { Award, Star, Trophy, BookOpen, Target, Zap, Lock, CheckCircle } from "lucide-react";
      ...
      <div className="relative p-6 z-10">
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className={cn(
              ...
              isUnlocked ? `bg-gradient-to-br ${config.gradient} text-white` : "bg-secondary text-muted-foreground"
            )}>
          ...
          {isUnlocked ? (
            <div className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm border border-yellow-200 dark:border-yellow-800">
              <Star className="w-3.5 h-3.5 fill-current" />
              <span>Certified</span>
            </div>
          ) : (
            <div className="bg-secondary text-muted-foreground px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="h-2.5 bg-secondary rounded-full overflow-hidden border border-border/50">
              <div
                className={cn("h-full rounded-full transition-all duration-1000", isUnlocked ? `bg-gradient-to-r ${config.gradient}` : "bg-muted-foreground/30")}
        ...
        {isUnlocked && (
          <div className="mt-6 pt-4 border-t border-border/50 flex justify-center">
            <button className={cn(
              "text-sm font-bold flex items-center gap-2 transition-colors hover:underline decoration-2 underline-offset-4",
              config.color
            )}>
