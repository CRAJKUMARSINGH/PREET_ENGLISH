...
export function QuickRevision() {
  ...
  const question = quickQuizQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / quickQuizQuestions.length) * 100;
  ...
  const getScoreMessage = () => {
    const percentage = (score / quickQuizQuestions.length) * 100;
    if (percentage >= 80) return { message: 'Excellent! बहुत बढ़िया!', emoji: '🎉' };
    if (percentage >= 60) return { message: 'Good job! अच्छा काम!', emoji: '👍' };
    if (percentage >= 40) return { message: 'Keep practicing! अभ्यास जारी रखें!', emoji: '💪' };
  ...
  return (
    <div className="quick-revision space-y-6">
      {/* Header */}
      <Card>
        <CardHeader>
            ...
            {question.options.map((option, index) => (
              <Button
                ...
                className={cn(
                  ...
                  showResult && index === question.correctAnswer && "bg-green-100 border-green-500 dark:bg-green-950/20",
                  showResult && selectedAnswer === index && index !== question.correctAnswer && "bg-red-100 border-red-500 dark:bg-red-950/20",
                  !showResult && selectedAnswer === index && "border-blue-500"
                )}
