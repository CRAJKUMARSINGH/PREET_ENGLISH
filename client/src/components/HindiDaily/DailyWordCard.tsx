    ...
    <Card className="border-2 border-indigo-200 dark:border-indigo-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center gap-3">
            ...
            <Button 
              ...
              className={favorites.includes(currentWord.id) ? "text-red-500" : ""}
            >
              <Heart className={`h-5 w-5 ${favorites.includes(currentWord.id) ? "fill-red-500" : ""}`} />
            </Button>
          </div>
          ...
          <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
            ...
            <div className="flex flex-wrap gap-1">
              {currentWord.synonyms.map((syn, i) => (
                <Badge key={i} variant="secondary" className="bg-green-100 text-green-700">{syn}</Badge>
              ))}
            </div>
          </div>
          <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
            ...
            <div className="flex flex-wrap gap-1">
              {currentWord.antonyms.map((ant, i) => (
                <Badge key={i} variant="secondary" className="bg-red-100 text-red-700">{ant}</Badge>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-4">
          ...
          <Button 
            onClick={markLearned}
            disabled={learned.includes(currentWord.id)}
            className={learned.includes(currentWord.id) ? "bg-green-600" : "bg-indigo-600"}
          >
            {learned.includes(currentWord.id) ? (
              <><Star className="h-4 w-4 mr-1 fill-white" /> सीख लिया!</>
            ) : (
              <><Star className="h-4 w-4 mr-1" /> सीख लिया</>
            )}
          </Button>
