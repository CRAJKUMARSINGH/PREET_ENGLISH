...
export function PhrasesOfDay() {
    ...
    var _c = useState("All"), category = _c[0], setCategory = _c[1];
    var categories = ["All", "Greetings", "Conversation", "Office", "Idioms", "Polite", "Shopping", "Travel"];
    var filteredPhrases = category === "All"
        ? dailyPhrases
        : dailyPhrases.filter(function (p) { return p.category === category; });
    var currentPhrase = filteredPhrases[currentIndex % filteredPhrases.length];
    var speakPhrase = function () {
        var utterance = new SpeechSynthesisUtterance(currentPhrase.phrase);
        utterance.lang = "en-US";
        utterance.rate = 0.8;
        speechSynthesis.speak(utterance);
    };
    var copyPhrase = function () {
    ...
    return (<Card className="border-2 border-teal-200 dark:border-teal-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map(function (cat) { return (<Button key={cat} variant={category === cat ? "default" : "outline"} size="sm" onClick={function () { setCategory(cat); setCurrentIndex(0); }} className={category === cat ? "bg-teal-600" : ""}>
              {cat === "All" ? "सभी" : cat}
            </Button>); })}
        </div>
