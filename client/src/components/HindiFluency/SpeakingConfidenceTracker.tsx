...
export function SpeakingConfidenceTracker() {
  const [confidenceData, setConfidenceData] = useState<ConfidenceLevel[]>(confidenceLevels);
  const [challenges, setChallenges] = useState<DailyChallenge[]>(dailyChallenges);
  const [overallConfidence, setOverallConfidence] = useState(58);

  const currentMilestone = milestones.filter(m => m.level <= overallConfidence).pop();
  const nextMilestone = milestones.find(m => m.level > overallConfidence);

  const completeChallenge = (challengeId: string) => {
    setChallenges(prev => prev.map(c => {
      if (c.id === challengeId && !c.isCompleted) {
        setOverallConfidence(prev => Math.min(100, prev + 2));
  ...
  return (
    <div className="speaking-confidence-tracker space-y-6">
      {/* Overall Confidence */}
        ...
        <CardContent>
          <div className="flex justify-between">
            {milestones.map((milestone, index) => {
              const isAchieved = overallConfidence >= milestone.level;
              ...
              return (
                <div 
                  ...
                  className={cn(
                    ...
                    isAchieved ? "bg-green-50 dark:bg-green-950/20" : "bg-gray-50 dark:bg-gray-800",
          ...
          {challenges.map((challenge) => (
            ...
            >
              <div className="flex items-center gap-3">
                <Mic className={cn(
                  "w-8 h-8",
                  challenge.isCompleted ? "text-green-500" : "text-gray-400"
                )} />
                <div>
