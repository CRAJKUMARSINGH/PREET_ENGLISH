...
export function FillBlanks() {
  ...
  const categories = ["All", "Articles", "Prepositions", "Tenses", "Pronouns"];
  ...
  const speakSentence = () => {
    const fullSentence = currentQuestion.sentence.replace("___", currentQuestion.correctAnswer);
    ...
    <Card className="border-2 border-blue-200 dark:border-blue-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <select
            value={category}
            onChange={(e) => { setCategory(e.target.value); resetGame(); }}
            className="px-3 py-1 border rounded-lg bg-background text-sm"
          >
            {categories.map(cat => (
              <option key={cat} value={cat}>{cat === "All" ? "सभी" : cat}</option>
            ))}
          </select>
        </div>
          ...
          <p className="text-xl font-medium">
            {currentQuestion.sentence.split("___").map((part, i, arr) => (
              <span key={i}>
                {part}
                {i < arr.length - 1 && (
                  <span className="inline-block min-w-[80px] mx-1 px-2 py-1 bg-blue-100 dark:bg-blue-900/30 rounded border-b-2 border-blue-500 text-center">
