...
export function SpellingBee() {
    var _a = useState("easy"), difficulty = _a[0], setDifficulty = _a[1];
    var _b = useState(null), currentWord = _b[0], setCurrentWord = _b[1];
    ...
    var _f = useState(0), streak = _f[0], setStreak = _f[1];
    var _g = useState(0), bestStreak = _g[0], setBestStreak = _g[1];
    var _h = useState(0), wordsAttempted = _h[0], setWordsAttempted = _h[1];
    var getNewWord = function () {
        var filtered = spellingWords.filter(function (w) { return w.difficulty === difficulty; });
    ...
    var speakSentence = function () {
        if (!currentWord)
            return;
        var utterance = new SpeechSynthesisUtterance(currentWord.sentence);
        utterance.lang = "en-US";
        utterance.rate = 0.8;
        speechSynthesis.speak(utterance);
    };
    ...
    if (!currentWord)
        return null;
    var isCorrect = userInput.toLowerCase().trim() === currentWord.word.toLowerCase();
    return (<Card className="border-2 border-amber-200 dark:border-amber-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex gap-2 justify-center">
          {["easy", "medium", "hard"].map(function (level) { return (<Button key={level} variant={difficulty === level ? "default" : "outline"} size="sm" onClick={function () { return setDifficulty(level); }} className={difficulty === level ? "bg-amber-600" : ""}>
        ...
        <div className="space-y-2">
          ...
          <Input value={userInput} onChange={function (e) { return setUserInput(e.target.value); }} placeholder="Type the spelling..." className="text-center text-xl" disabled={showResult} onKeyDown={function (e) { return e.key === 'Enter' && !showResult && checkSpelling(); }} autoFocus/>
        </div>
        ...
        <div className="flex gap-3">
          {!showResult ? (<>
              ...
              <Button onClick={checkSpelling} disabled={!userInput.trim()} className="flex-1 bg-amber-600">
                जांचें (Check)
              </Button>
