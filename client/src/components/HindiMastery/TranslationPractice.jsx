...
import { Languages, CheckCircle, XCircle, RotateCcw, Lightbulb } from 'lucide-react';
import { cn } from '@/lib/utils';
var translationExercises = [
    {
        id: '1',
...
export function TranslationPractice() {
    ...
    var _b = useState(''), userAnswer = _b[0], setUserAnswer = _b[1];
    var _c = useState(false), showResult = _c[0], setShowResult = _c[1];
    var _d = useState(false), isCorrect = _d[0], setIsCorrect = _d[1];
    var _e = useState(false), showHints = _e[0], setShowHints = _e[1];
    var _f = useState({ correct: 0, total: 0 }), score = _f[0], setScore = _f[1];
    var _g = useState(0), streak = _g[0], setStreak = _g[1];
    var currentExercise = translationExercises[currentIndex];
    var checkAnswer = function () {
        var normalizedUser = userAnswer.toLowerCase().trim().replace(/[.,!?]/g, '');
        var normalizedCorrect = currentExercise.correctEnglish.toLowerCase().trim().replace(/[.,!?]/g, '');
        var normalizedAlternatives = currentExercise.alternativeAnswers.map(function (a) {
            return a.toLowerCase().trim().replace(/[.,!?]/g, '');
        });
        var correct = normalizedUser === normalizedCorrect ||
            normalizedAlternatives.includes(normalizedUser);
        setIsCorrect(correct);
        setShowResult(true);
        setScore(function (prev) { return ({
    ...
    var accuracy = score.total > 0 ? Math.round((score.correct / score.total) * 100) : 0;
    return (<div className="translation-practice space-y-6">
      {/* Header Stats */}
      <Card>
        <CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="user-input">
            ...
            <Textarea value={userAnswer} onChange={function (e) { return setUserAnswer(e.target.value); }} placeholder="Type your English translation here..." className="min-h-[100px]" disabled={showResult}/>
          </div>

          {/* Hints */}
          {showHints && !showResult && (<div className="hints bg-yellow-50 dark:bg-yellow-950/20 p-4 rounded-lg">
              ...
              <ul className="list-disc list-inside text-sm text-yellow-600 dark:text-yellow-400">
                {currentExercise.hints.map(function (hint, index) { return (<li key={index}>{hint}</li>); })}
              </ul>
            </div>)}

          {/* Result */}
          {showResult && (<div className={cn('result p-4 rounded-lg', isCorrect ? 'bg-green-50 dark:bg-green-950/20' : 'bg-red-50 dark:bg-red-950/20')}>
              ...
              {currentExercise.alternativeAnswers.length > 0 && (<div className="alternatives mb-3">
                  <div className="text-sm font-medium mb-1">Alternative Answers:</div>
                  <ul className="list-disc list-inside text-sm text-muted-foreground">
                    {currentExercise.alternativeAnswers.map(function (alt, index) { return (<li key={index}>{alt}</li>); })}
                  </ul>
                </div>)}
              ...
              <div className="common-mistakes">
                ...
                <ul className="list-disc list-inside text-sm text-red-500">
                  {currentExercise.commonMistakes.map(function (mistake, index) { return (<li key={index}>{mistake}</li>); })}
                </ul>
              </div>
            </div>)}
          ...
          <div className="flex gap-3">
            {!showResult ? (<>
                ...
                <Button variant="outline" onClick={function () { return setShowHints(!showHints); }}>
