...
export function ComprehensionQuiz() {
    var _a = useState(null), selectedQuiz = _a[0], setSelectedQuiz = _a[1];
    ...
    var handleNext = function () {
        if (selectedAnswer === null || !selectedQuiz)
            return;
        var newAnswers = __spreadArray(__spreadArray([], answers, true), [selectedAnswer], false);
    ...
    if (!selectedQuiz) {
        ...
        <CardContent className="p-6">
          <div className="grid gap-4">
            {quizzes.map(function (quiz, idx) { return (<div key={idx} onClick={function () { return setSelectedQuiz(quiz); }} className="p-4 border-2 border-blue-100 dark:border-blue-800 rounded-xl hover:border-blue-300 dark:hover:border-blue-600 cursor-pointer transition-all hover:shadow-lg">
    ...
    if (showResult) {
        var percentage = Math.round((score / selectedQuiz.questions.length) * 100);
        return (<Card className="border-2 border-blue-200 dark:border-blue-800">
        <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20">
          <CardTitle className="flex items-center gap-2 text-blue-700 dark:text-blue-300">
        ...
        <CardContent className="p-6 text-center space-y-6">
          ...
          {percentage >= 80 && (<Badge className="text-lg py-2 px-4 bg-green-500">🎉 बहुत बढ़िया! Excellent!</Badge>)}
          {percentage >= 50 && percentage < 80 && (<Badge className="text-lg py-2 px-4 bg-yellow-500">👍 अच्छा! Good!</Badge>)}
          {percentage < 50 && (<Badge className="text-lg py-2 px-4 bg-orange-500">📚 और अभ्यास करें! Keep practicing!</Badge>)}

          <div className="space-y-3 text-left">
            {selectedQuiz.questions.map(function (q, idx) { return (<div key={idx} className="p-3 rounded-lg border flex items-center gap-3">
                {answers[idx] === q.correctAnswer ? (<CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0"/>) : (<XCircle className="h-5 w-5 text-red-500 flex-shrink-0"/>)}
                <div>
    ...
    return (<Card className="border-2 border-blue-200 dark:border-blue-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="grid gap-3">
          {question.options.map(function (option, idx) { return (<button key={idx} onClick={function () { return handleAnswer(idx); }} className={"p-4 rounded-xl border-2 text-left transition-all ".concat(selectedAnswer === idx
