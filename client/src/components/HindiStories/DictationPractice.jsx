...
export function DictationPractice() {
    var _a = useState("easy"), currentLevel = _a[0], setCurrentLevel = _a[1];
    ...
    var _g = useState(0), attempts = _g[0], setAttempts = _g[1];
    var filteredSentences = sentences.filter(function (s) { return s.level === currentLevel; });
    var currentSentence = filteredSentences[currentIndex];
    var speakSentence = function (speed) {
        if (speed === void 0) { speed = 0.8; }
        var utterance = new SpeechSynthesisUtterance(currentSentence.english);
    ...
    var checkAnswer = function () {
        ...
        var normalizedInput = userInput.trim().toLowerCase().replace(/[.,!?]/g, "");
        var normalizedAnswer = currentSentence.english.toLowerCase().replace(/[.,!?]/g, "");
    ...
    var isCorrect = userInput.trim().toLowerCase().replace(/[.,!?]/g, "") ===
    ...
    return (<Card className="border-2 border-purple-200 dark:border-purple-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex gap-2 justify-center">
          {["easy", "medium", "hard"].map(function (level) { return (<Button key={level} variant={currentLevel === level ? "default" : "outline"} onClick={function () {
        ...
        <div className="flex flex-col items-center gap-4 p-6 bg-white dark:bg-slate-800 rounded-xl border">
          ...
          <div className="flex gap-3">
            <Button onClick={function () { return speakSentence(0.6); }} variant="outline">
              <Volume2 className="h-4 w-4 mr-2"/> धीमा (Slow)
            </Button>
            <Button onClick={function () { return speakSentence(0.8); }} className="bg-purple-600">
              <Volume2 className="h-4 w-4 mr-2"/> सामान्य (Normal)
            </Button>
            <Button onClick={function () { return speakSentence(1); }} variant="outline">
        ...
        {/* Word Hints */}
        {showHint && (<div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
            <p className="text-sm text-green-600 dark:text-green-400 mb-2">मुख्य शब्द (Key Words):</p>
            <div className="flex gap-2 flex-wrap">
              {currentSentence.hints.map(function (hint, idx) { return (<Badge key={idx} variant="secondary" className="bg-green-100 dark:bg-green-800">
                  {hint}
                </Badge>); })}
            </div>
          </div>)}
        ...
        <div className="space-y-2">
          ...
          <Textarea value={userInput} onChange={function (e) { return setUserInput(e.target.value); }} placeholder="Type what you hear..." className="min-h-[100px] text-lg" disabled={showResult}/>
        </div>
