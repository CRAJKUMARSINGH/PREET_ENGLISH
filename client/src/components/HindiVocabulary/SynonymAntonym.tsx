...
interface WordPair {
  ...
  word: string;
  hindi: string;
  synonyms: { word: string; hindi: string }[];
  antonyms: { word: string; hindi: string }[];
...
export function SynonymAntonym() {
  ...
  const [gameMode, setGameMode] = useState<"synonym" | "antonym">("synonym");
  ...
  const getRandomWord = () => {
    const filtered = difficulty === "all" ? wordPairs : wordPairs.filter(w => w.difficulty === difficulty);
    ...
    // Generate options
    const correctOptions = Math.random() > 0.5 ? word.synonyms : word.antonyms;
    const incorrectOptions = Math.random() > 0.5 ? word.antonyms : word.synonyms;
    const selectedCorrect = correctOptions.slice(0, 3);
    const selectedIncorrect = incorrectOptions.slice(0, 3);
    const options = [...selectedCorrect, ...selectedIncorrect]
      .map(opt => opt.word)
  ...
  const checkAnswers = () => {
    ...
    const correctWords = gameMode === "synonym" 
      ? currentWord.synonyms.map(s => s.word)
      : currentWord.antonyms.map(a => a.word);
    
    const correctCount = selectedOptions.filter(opt => correctWords.includes(opt)).length;
    const incorrectCount = selectedOptions.filter(opt => !correctWords.includes(opt)).length;
    const questionScore = Math.max(0, (correctCount * 2) - incorrectCount);
    setScore(score + questionScore);
  };

  const getOptionStatus = (option: string) => {
    if (!currentWord || !showResult) return "default";
    const correctWords = gameMode === "synonym" 
      ? currentWord.synonyms.map(s => s.word)
      : currentWord.antonyms.map(a => a.word);
  ...
  const getOptionClass = (status: string) => {
    switch (status) {
    ...
    <Card className="border-2 border-purple-200 dark:border-purple-800">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="text-center space-y-4">
          <div className="p-6 bg-purple-100 dark:bg-purple-900/30 rounded-xl">
            <div className="flex items-center justify-center gap-3 mb-2">
              ...
              <Button onClick={() => speakWord(currentWord.word)} size="sm" variant="ghost">
            ...
            return (
              <button
                ...
                className={`p-3 rounded-xl border-2 font-medium transition-all ${
                  isSelected && !showResult ? "border-purple-500 bg-purple-50 dark:bg-purple-900/30" : ""
        ...
        <div className="flex gap-3">
          {!showResult ? (
            <Button onClick={checkAnswers} disabled={selectedOptions.length === 0} className="w-full">
              जांचें
            </Button>
          ) : (
