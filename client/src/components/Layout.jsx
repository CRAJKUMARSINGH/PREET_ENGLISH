...
import { BookOpen, Home, Trophy, User, Users, Video, Menu, X, Brain, BookMarked, Gamepad2, Calendar, Library, MessageCircle, Bot, Search, Zap, Star } from "lucide-react";
...
export function Layout(_a) {
    ...
    var _c = useState(false), isSearchOpen = _c[0], setIsSearchOpen = _c[1];
    ...
    var toggleLanguage = function () {
        var newLang = i18n.language === "en" ? "hi" : "en";
        i18n.changeLanguage(newLang);
    };
    return (<div className="min-h-screen bg-background text-foreground flex font-sans selection:bg-primary/30">

      {/* Mobile Header */}
      <div className={cn("fixed top-0 left-0 right-0 z-50 md:hidden transition-all duration-300 border-b border-border/50", scrolled ? "bg-background/80 backdrop-blur-md shadow-sm" : "bg-transparent")}>
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-2">
            <button onClick={function () { return setIsMobileMenuOpen(!isMobileMenuOpen); }} className="p-2 hover:bg-muted rounded-full transition-colors">
              {isMobileMenuOpen ? <X className="h-6 w-6"/> : <Menu className="h-6 w-6"/>}
            </button>
            <SaraswatiLogo size="xs"/>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={function () { return setIsSearchOpen(true); }} className="p-2 hover:bg-muted rounded-full transition-colors">
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-40 w-[280px] flex flex-col transition-transform duration-500 ease-in-out md:translate-x-0 md:sticky md:top-0 md:h-screen border-r border-border/50", "glass-panel md:bg-card/50", isMobileMenuOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <div className="px-4 mb-6">
          <button onClick={function () { return setIsSearchOpen(true); }} className="w-full relative group flex items-center gap-3 px-4 py-3 rounded-xl bg-secondary/50 hover:bg-secondary/80 border border-transparent hover:border-primary/20 transition-all duration-300 text-sm font-medium text-muted-foreground hover:text-foreground hover:shadow-lg hover:shadow-primary/5">
        ...
        <nav className="flex-1 px-4 space-y-1.5 overflow-y-auto no-scrollbar py-2">
          {navItems.map(function (item) {
            ...
            return (<Link key={item.href} href={item.href}>
                <div onClick={function () { return setIsMobileMenuOpen(false); }} className={cn("relative flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-300 cursor-pointer font-medium text-sm overflow-hidden group", isActive
                    ? "bg-primary/10 text-primary font-bold shadow-none"
                    : "text-muted-foreground hover:text-foreground hover:bg-secondary/50")}>
                  {isActive && (<div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-primary rounded-r-full shadow-[0_0_10px_2px_rgba(28,231,131,0.5)]"/>)}
                  <item.icon className={cn("h-5 w-5 transition-transform duration-300 group-hover:scale-110", isActive ? "text-primary drop-shadow-[0_0_5px_rgba(28,231,131,0.5)]" : "text-muted-foreground group-hover:text-foreground")}/>
                  <span className="relative z-10">{item.label}</span>
                  {item.highlight && !isActive && (<span className="ml-auto w-2 h-2 rounded-full bg-primary/50 animate-pulse"/>)}
                </div>
              </Link>);
        })}
          ...
          <div className="pt-6 mt-6 border-t border-border/50">
            ...
            {secondaryNavItems.map(function (item) {
            var isActive = location === item.href;
            return (<Link key={item.href} href={item.href}>
                  <div onClick={function () { return setIsMobileMenuOpen(false); }} className={cn("flex items-center gap-3.5 px-4 py-2.5 rounded-xl transition-colors cursor-pointer text-sm font-medium", isActive ? "text-primary bg-primary/5" : "text-muted-foreground hover:text-foreground hover:bg-secondary/30")}>
          ...
          <div className="pt-4 mt-4 border-t border-border/50">
            ...
            {labsNavItems.map(function (item) {
            var isActive = location === item.href || location.startsWith(item.href);
            return (<Link key={item.href} href={item.href}>
                  <div onClick={function () { return setIsMobileMenuOpen(false); }} className={cn("flex items-center gap-3.5 px-4 py-2.5 rounded-xl transition-colors cursor-pointer text-sm font-medium", isActive ? "text-purple-500 bg-purple-500/10" : "text-muted-foreground hover:text-foreground hover:bg-secondary/30")}>
      ...
      {isMobileMenuOpen && (<div className="fixed inset-0 bg-black/60 z-30 md:hidden backdrop-blur-sm animate-in fade-in duration-300" onClick={function () { return setIsMobileMenuOpen(false); }}/>)}
