...
export function LeaderboardCard(_a) {
    var entries = _a.entries, currentUserRank = _a.currentUserRank;
    ...
    var getRankBg = function (rank, isCurrentUser) {
        if (isCurrentUser)
            return "bg-primary/10 border-primary/20 shadow-sm";
        switch (rank) {
            case 1:
    ...
    return (<div className="glass-card rounded-[2rem] p-6 border-border/50">
      <div className="flex items-center justify-between mb-6">
        ...
        {currentUserRank && (<div className="text-right">
      ...
      <div className="space-y-3">
        {entries.slice(0, 5).map(function (entry) { return (<div key={entry.rank} className={"flex items-center justify-between p-3 rounded-xl transition-all hover:scale-[1.02] ".concat(entry.isCurrentUser
                ? "bg-primary/20 border border-primary/50 shadow-lg shadow-primary/10"
                : "bg-white/5 hover:bg-white/10 border border-white/5")}>
            <div className="flex items-center gap-3">
              <div className={"w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ".concat(entry.rank === 1 ? "bg-yellow-400 text-yellow-900 shadow-md shadow-yellow-400/20" :
              ...
              <div>
                <p className={"text-sm font-bold ".concat(entry.isCurrentUser ? "text-primary bg-primary/10 px-2 py-0.5 rounded-md" : "text-foreground")}>
