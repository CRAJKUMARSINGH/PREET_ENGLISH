...
interface MistakeCorrectionProps {
    mistake: string;
    correction: string;
    explanation: string;
...
export function MistakeCorrection({
    mistake,
    correction,
    explanation,
    hindiExplanation,
    className,
}: MistakeCorrectionProps) {
    return (
        <Card className={cn(
            "overflow-hidden border-none bg-white dark:bg-slate-900 shadow-xl ring-1 ring-slate-200 dark:ring-slate-800",
            className
        )}>
            ...
            <CardContent className="p-6">
                ...
                <div className="grid gap-4">
                    ...
                    <div className="mt-2 pl-4 border-l-2 border-slate-200 dark:border-slate-800 space-y-2">
                        ...
                        {hindiExplanation && (
                            <p className="text-sm font-hindi text-slate-500 dark:text-slate-500 italic">
                                {hindiExplanation}
                            </p>
                        )}
                    </div>
