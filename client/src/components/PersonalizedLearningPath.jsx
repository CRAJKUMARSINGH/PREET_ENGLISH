...
export function PersonalizedLearningPath() {
    ...
    ]), goals = _a[0], setGoals = _a[1];
    var _b = useState('beginner'), userLevel = _b[0], setUserLevel = _b[1];
    var _c = useState(5), weeklyGoal = _c[0], setWeeklyGoal = _c[1]; // lessons per week
    var _d = useState(3), completedThisWeek = _d[0], setCompletedThisWeek = _d[1];
    useEffect(function () {
        var stored = localStorage.getItem('learning_path');
        if (stored) {
    ...
    var getDifficultyLabel = function (difficulty) {
        switch (difficulty) {
    ...
    return (<div className="space-y-6">
      ...
      <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20">
        ...
        <CardContent className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            ? "".concat(weeklyGoal - completedThisWeek, " more lessons to reach your weekly goal!")
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-4">
            <div className={"w-16 h-16 rounded-full ".concat(getDifficultyColor(userLevel), " flex items-center justify-center text-white text-2xl font-bold")}>
      ...
      <div className="space-y-4">
        ...
        {goals.map(function (goal) { return (<Card key={goal.id} className="hover:shadow-lg transition-shadow">
            <CardContent className="pt-6">
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  ...
                  <span className={"px-3 py-1 rounded-full text-xs font-medium text-white ".concat(getDifficultyColor(goal.difficulty))}>
                ...
                <Button className="w-full" variant={goal.progress > 0 ? 'default' : 'outline'}>
                  {goal.progress === 100 ? (<>
                      ...
                      Completed!
                    </>) : goal.progress > 0 ? ('Continue Learning') : ('Start Learning')}
                </Button>
              </div>
            </CardContent>
          </Card>); })}
