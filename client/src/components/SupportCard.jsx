...
var grammarCategoryMap = {
    articles: "Articles (a, an, the)",
    prepositions: "Prepositions (in, on, at)",
    present_continuous: "Present Continuous Overuse",
};
export function getGrammarEntryForKey(key) {
    var _a;
    var categoryName = grammarCategoryMap[key];
    return (_a = commonGrammarMistakes.find(function (m) { return m.category === categoryName; })) !== null && _a !== void 0 ? _a : null;
}
export function inferGrammarCategoryFromText(text) {
...
export function SupportCard(_a) {
    var categoryKey = _a.categoryKey;
    if (!categoryKey)
        return null;
    var entry = getGrammarEntryForKey(categoryKey);
    if (!entry)
        return null;
    var first = entry.mistakes[0];
    return (<Card className={cn("mt-4 overflow-hidden", "glass-card border-primary/20", "bg-gradient-to-br from-primary/5 via-background to-primary/5")}>
      ...
      <CardContent className="pt-0 space-y-4">
        ...
        {first && (<div className="rounded-xl border border-border/50 overflow-hidden">
            {/* Wrong Example */}
