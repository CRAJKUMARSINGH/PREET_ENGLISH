...
export function VocabularyBuilder() {
    ...
    var _a = useState("daily_routine"), expandedCategory = _a[0], setExpandedCategory = _a[1];
    var _b = useState(new Set()), learnedWords = _b[0], setLearnedWords = _b[1];
    var speakWord = function (text) {
        speak({ text: text, lang: 'en-US', rate: 0.8 });
    };
    var toggleLearned = function (wordId) {
        var newLearned = new Set(learnedWords);
    ...
    var totalWords = vocabularyData.reduce(function (acc, cat) { return acc + cat.words.length; }, 0);
    var learnedCount = learnedWords.size;
    return (<div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-500 to-indigo-500 p-6 text-white">
        ...
        <div className="bg-white/20 rounded-xl p-3">
          ...
          <div className="h-2 bg-white/30 rounded-full overflow-hidden">
            <div className="h-full bg-white rounded-full transition-all duration-500" style={{ width: "".concat((learnedCount / totalWords) * 100, "%") }}/>
          </div>
        </div>
      </div>
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        {vocabularyData.map(function (category) { return (<div key={category.id}>
            {/* Category Header */}
            <button onClick={function () { return setExpandedCategory(expandedCategory === category.id ? null : category.id); }} className="w-full p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            ...
            {/* Words List */}
            {expandedCategory === category.id && (<div className="px-4 pb-4 space-y-2">
                {category.words.map(function (word, index) {
                    var wordId = "".concat(category.id, "-").concat(index);
                    var isLearned = learnedWords.has(wordId);
                    return (<div key={index} className={cn("p-3 rounded-xl border transition-all", isLearned
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <button onClick={function () { return speakWord(word.english); }} className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition-colors">
                            ...
                            {word.example && (<p className="text-sm text-slate-600 dark:text-slate-400 mt-1 bg-white dark:bg-slate-900 px-2 py-1 rounded">
                                "{word.example}"
                              </p>)}
                          </div>
                        </div>
                        <button onClick={function () { return toggleLearned(wordId); }} className={cn("p-2 rounded-full transition-all", isLearned
