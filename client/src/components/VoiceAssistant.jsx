...
export function VoiceAssistant() {
    ...
    var speak = function (text) {
        if ('speechSynthesis' in window) {
            ...
            utterance.rate = 0.9;
            utterance.onend = function () { return setIsSpeaking(false); };
      ...
      <Card className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-center">
            <Button onClick={isListening ? stopListening : startListening} size="lg" className={"w-24 h-24 rounded-full ".concat(isListening
          ...
          {/* Transcript */}
          {transcript && (<div className="bg-white dark:bg-slate-800 rounded-lg p-4 border">
              <p className="text-sm text-muted-foreground mb-1">You said:</p>
              <p className="font-medium">{transcript}</p>
            </div>)}

          {/* Response */}
          {response && (<div className="bg-purple-100 dark:bg-purple-900/30 rounded-lg p-4 border border-purple-200 dark:border-purple-800">
              <div className="flex items-start gap-2">
                <Volume2 className={"w-5 h-5 text-purple-600 ".concat(isSpeaking ? 'animate-pulse' : '')}/>
                <div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {commands.map(function (cmd, index) { return (<div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
