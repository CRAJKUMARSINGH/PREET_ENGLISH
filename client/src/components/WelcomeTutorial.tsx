...
export function WelcomeTutorial() {
    ...
    useEffect(() => {
        ...
        const hasSeen = localStorage.getItem("pre_english_onboarding_completed");
        if (!hasSeen) {
            setTimeout(() => setIsOpen(true), 1000); // Delay for dramatic entrance
        }
    }, []);
        ...
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    ...
                    >
                        {/* Background Gradient */}
                        <div className={`absolute top-0 inset-x-0 h-32 ${currentStep.color} transition-colors duration-500`} />
                        ...
                        <div className="relative pt-8 px-8 pb-8">
                            ...
                            <div className="flex justify-center mb-6">
                                <motion.div
                                    ...
                                    className={`w-20 h-20 rounded-full ${currentStep.color} shadow-lg border-4 border-white dark:border-slate-800 flex items-center justify-center transition-colors duration-500`}
                                >
                                    {currentStep.icon}
                                </motion.div>
                            </div>
                            ...
                            <div className="flex justify-center gap-2 mb-8 mt-4">
                                {steps.map((_, i) => (
                                    <div
                                        key={i}
                                        className={`w-2.5 h-2.5 rounded-full transition-colors duration-300 ${i === step ? currentStep.color : "bg-slate-200 dark:bg-slate-700"}`}
                                    />
                                ))}
                            </div>
                            ...
                            >
                                {step < steps.length - 1 ? (
                                    ...
                                    <span className="flex items-center gap-2">Get Started <Sparkles className="w-5 h-5" /></span>
                                )}
                            </Button>
                        </div>
