...
import { Users, BookOpen, Zap, MessageSquare, TrendingUp, Activity } from "lucide-react";
import { Badge } from "@/components/ui/badge";
export default function AnalyticsDashboard() {
    var _a = useState(analytics.getLiveStats()), stats = _a[0], setStats = _a[1];
    ...
    return (<Layout>
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {cards.map(function (card, i) { return (<Card key={i} className="border-border/50 shadow-sm hover:shadow-md transition-shadow">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">
                                    {card.title}
                                </CardTitle>
                                <div className={"p-2 rounded-full ".concat(card.bg)}>
                                    <card.icon className={"h-4 w-4 ".concat(card.color)}/>
                                </div>
                            </CardHeader>
                            <CardContent>
