...
export default function Chat() {
    ...
    var _b = useChatStream(), messages = _b.messages, sendMessage = _b.sendMessage, isLoading = _b.isLoading;
    ...
    return (<ErrorBoundary>
                ...
                <div className="flex h-[calc(100vh-12rem)] max-h-[700px] bg-white dark:bg-black/40 backdrop-blur-xl rounded-[2.5rem] border border-white/20 overflow-hidden shadow-2xl relative">
                    ...
                    <div className="flex-1 flex flex-col relative z-10">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {messages.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-center p-6 animate-fade-in-up">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
                                        <button onClick={function () { return sendMessage("Translate 'Success' to Hindi"); }} className="p-3 bg-secondary/50 hover:bg-secondary rounded-xl text-sm font-medium transition-colors text-left">
                                            🤔 Translate "Success"
                                        </button>
                                        <button onClick={function () { return sendMessage("How to introduce myself?"); }} className="p-3 bg-secondary/50 hover:bg-secondary rounded-xl text-sm font-medium transition-colors text-left">
                                            👋 How to introduce myself?
                                        </button>
                                        <button onClick={function () { return sendMessage("Correct this: I goes to market"); }} className="p-3 bg-secondary/50 hover:bg-secondary rounded-xl text-sm font-medium transition-colors text-left">
                                            📝 Correct: "I goes to market"
                                        </button>
                                        <button onClick={function () { return sendMessage("Tell me a story in English"); }} className="p-3 bg-secondary/50 hover:bg-secondary rounded-xl text-sm font-medium transition-colors text-left">
                                            📖 Tell me a story
                                        </button>
                                    </div>
                                </div>) : (<>
                                    {messages.map(function (msg) { return (<div key={msg.id} className={cn("flex gap-4 animate-in fade-in slide-in-from-bottom-2", msg.sender === 'user' ? "flex-row-reverse" : "flex-row")}>
                                            ...
                                            <div className={cn("max-w-[80%] p-4 rounded-2xl shadow-sm", msg.sender === 'user'
            ...
            if (input.trim()) {
                ...
                setInput("");
                                ...
                                <Input value={input} onChange={function (e) { return setInput(e.target.value); }} placeholder="Type your message... / अपना संदेश टाइप करें..." className="flex-1 h-12 rounded-xl pl-4 pr-12 bg-white/80 dark:bg-slate-900/80 border-0 shadow-inner focus-visible:ring-1 focus-visible:ring-primary" disabled={isLoading}/>
