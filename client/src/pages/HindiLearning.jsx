...
import { BookOpen, Mic, Globe, Users, Briefcase, ShoppingCart, MessageCircle, GraduationCap, Target, TrendingUp } from 'lucide-react';
...
import { pronunciationChallenges, commonGrammarMistakes, culturalContextData, dailyEnglishPhrases } from '@/data/hindiLearningData';
export default function HindiLearning() {
    var _a = useState('overview'), activeTab = _a[0], setActiveTab = _a[1];
    var _b = useState('th_sounds'), selectedPronunciationChallenge = _b[0], setSelectedPronunciationChallenge = _b[1];
    return (<div className="hindi-learning-page container mx-auto px-4 py-8">
      {/* Header */}
          ...
          <div className="pronunciation-section">
            ...
            <div className="challenge-selector mb-6">
              <div className="flex flex-wrap gap-2 mb-4">
                {Object.entries(pronunciationChallenges).map(function (_a) {
            var key = _a[0], challenge = _a[1];
            return (<Button key={key} variant={selectedPronunciationChallenge === key ? "default" : "outline"} onClick={function () { return setSelectedPronunciationChallenge(key); }}>
                    {challenge.title}
                  </Button>);
        })}
              </div>
              ...
              {selectedPronunciationChallenge && (<Card className="challenge-details">
                  <CardHeader>
                    <CardTitle>
                      {pronunciationChallenges[selectedPronunciationChallenge].title}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-muted-foreground mb-4">
                      {pronunciationChallenges[selectedPronunciationChallenge].description}
                    </p>
                    
                    <div className="words-grid grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {pronunciationChallenges[selectedPronunciationChallenge].words.map(function (wordData, index) { return (<PronunciationGuide key={index} englishWord={wordData.word} hindiComparison={wordData.hindi} soundTip={wordData.tip}/>); })}
                    </div>
                  </CardContent>
                </Card>)}
            </div>
            ...
            <PronunciationCoach targetWord="think" hindiComparison="सोचना" soundTips={[
        ...
        <TabsContent value="grammar" className="space-y-6">
          <div className="grammar-section">
            ...
            <div className="grammar-mistakes space-y-6">
              {commonGrammarMistakes.map(function (category, index) { return (<GrammarExplanationCard key={index} rule={category.category} hindiExplanation={category.hindiExplanation} englishExamples={category.mistakes.map(function (m) { return "\u2705 ".concat(m.correct); })} hindiComparison={category.mistakes[0].explanation}/>); })}
            </div>
            ...
            <div className="mistakes-examples mt-8">
              ...
              <div className="grid gap-4">
                {commonGrammarMistakes[0].mistakes.map(function (mistake, index) { return (<CommonMistakeCard key={index} wrongSentence={mistake.wrong} correctSentence={mistake.correct} hindiExplanation={mistake.explanation}/>); })}
              </div>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="culture" className="space-y-6">
          <div className="culture-section">
            ...
            <div className="cultural-contexts space-y-6">
              {culturalContextData.map(function (context, index) { return (<CulturalContextCard key={index} englishPhrase={context.englishPhrase} hindiEquivalent={context.hindiEquivalent} culturalContext={context.culturalContext} formalUsage={context.formalUsage} informalUsage={context.informalUsage} businessContext={context.businessContext} socialContext={context.socialContext} dosDonts={context.dosDonts} indianEnglishVariation={context.indianEnglishVariation}/>); })}
            </div>
          ...
          <div className="business-section">
            ...
            <div className="business-scenarios grid gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <HindiExplanation englishText="Professional Email Structure" hindiExplanation="व्यावसायिक ईमेल की संरचना भारतीय style से अलग होती है" tip="'Respected Sir' के बजाय 'Dear Mr./Ms.' का उपयोग करें"/>
              ...
              <Card>
                ...
                <CardContent>
                  <CommonMistakeCard wrongSentence="Hello, myself Raj from ABC Company." correctSentence="Good morning, this is Raj from ABC Company." hindiExplanation="'Myself' का उपयोग introduction के लिए नहीं करते। 'This is' या 'My name is' कहें।"/>
                </CardContent>
              </Card>
            </div>
          </div>
        ...
        <TabsContent value="daily" className="space-y-6">
          <div className="daily-section">
            ...
            <div className="daily-categories space-y-6">
              {dailyEnglishPhrases.map(function (category, index) { return (<Card key={index}>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="phrases-grid space-y-4">
                      {category.phrases.map(function (phrase, phraseIndex) { return (<HindiLearningCard key={phraseIndex} englishSentence={phrase.english} hindiMeaning={phrase.hindi} pronunciation={phrase.pronunciation} difficulty="easy" category={category.category} tips={[phrase.usage]}/>); })}
                    </div>
                  </CardContent>
                </Card>); })}
            </div>
