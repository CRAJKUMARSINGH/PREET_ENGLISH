...
import { Sparkles, BookOpen, Mic, CheckCircle, Loader2, Trophy, Crown, ArrowRight, Play, } from "lucide-react";
...
export default function Home() {
    ...
    var leaderboardEntries = (leaderboard !== null && leaderboard !== void 0 ? leaderboard : []).map(function (row, index) {
        ...
        return ({
            rank: index + 1,
            username: ((_a = row.user) === null || _a === void 0 ? void 0 : _a.username) || 'Unknown',
            xpEarned: Number((_b = row.xpEarned) !== null && _b !== void 0 ? _b : 0),
            lessonsCompleted: Number((_c = row.lessonsCompleted) !== null && _c !== void 0 ? _c : 0),
            isCurrentUser: ((_d = row.user) === null || _d === void 0 ? void 0 : _d.id) === 1, // Assumption: Mock user ID 1 for now, simpler
      ...
      <section className="mb-12 relative overflow-hidden rounded-[2.5rem] bg-gradient-to-r from-amber-600 to-amber-800 p-8 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white/30 overflow-hidden shadow-xl shrink-0">
            <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=800&auto=format&fit=crop&q=60" alt="Celebrity Authenticity" className="w-full h-full object-cover"/>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        ...
        <div className="lg:col-span-2 glass-card rounded-[2rem] p-6 relative overflow-hidden group border-border/50">
          ...
          <div className="h-3 bg-secondary rounded-full overflow-hidden">
            <div className="h-full bg-primary shadow-[0_0_10px_rgba(28,231,131,0.5)] transition-all duration-1000" style={{ width: "".concat(percentage, "%") }}/>
          </div>
          ...
          <div className="mt-6 flex justify-between items-center">
            ...
            <Link href={"/lesson/".concat(completedCount + 1)}>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex bg-secondary/50 p-1 rounded-xl">
              <button onClick={function () { return setActiveTab('lessons'); }} className={cn("px-4 py-1.5 rounded-lg text-xs font-bold transition-all", activeTab === 'lessons' ? "bg-white dark:bg-black text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                {t('lessons')}
              </button>
              <button onClick={function () { return setActiveTab('scenarios'); }} className={cn("px-4 py-1.5 rounded-lg text-xs font-bold transition-all", activeTab === 'scenarios' ? "bg-white dark:bg-black text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                {t('roleplay')}
              </button>
              <button onClick={function () { return setActiveTab('stories'); }} className={cn("px-4 py-1.5 rounded-lg text-xs font-bold transition-all", activeTab === 'stories' ? "bg-white dark:bg-black text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                Stories
              </button>
            </div>
          </div>
          ...
          <div className="min-h-[400px]">
            {isLoading ? (<div className="flex justify-center items-center h-60">
              ...
              </div>) : activeTab === 'lessons' ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {lessons === null || lessons === void 0 ? void 0 : lessons.sort(function (a, b) { return a.order - b.order; }).slice(0, 6).map(function (lesson) { return (<LessonCard key={lesson.id} lesson={lesson} progress={progress === null || progress === void 0 ? void 0 : progress.find(function (p) { return p.lessonId === lesson.id; })}/>); })}
                <Link href="/lessons">
              ...
              </div>) : activeTab === 'scenarios' ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {scenarios.map(function (scenario) { return (<ScenarioCard key={scenario.id} {...scenario}/>); })}
              </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {hindiStoriesData.map(function (story) { return (<HindiStoryCard key={story.id} story={story} onRead={function (storyId) {
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="glass-card p-6 rounded-3xl">
            ...
            {leaderboardEntries.length > 0 ? (<LeaderboardCard entries={leaderboardEntries} currentUserRank={currentUserRank}/>) : (<p className="text-sm text-muted-foreground text-center py-4">Compete to be #1!</p>)}
          </div>

          <div className="glass-card p-6 rounded-3xl">
            <h3 className="font-bold mb-4 text-sm uppercase tracking-wider">Achievements</h3>
            <div className="grid grid-cols-2 gap-2">
              {achievements.slice(0, 4).map(function (achievement, index) { return (<AchievementBadge key={index} {...achievement}/>); })}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link href="/labs/video-call">
            <div className="group relative overflow-hidden rounded-[2rem] bg-slate-900 aspect-[4/3] cursor-pointer shadow-xl">
              <img src="https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&auto=format&fit=crop&q=60" className="absolute inset-0 w-full h-full object-cover opacity-60 transition-transform duration-700 group-hover:scale-110" alt="Video Call"/>
          ...
          <Link href="/labs/reader">
            <div className="group relative overflow-hidden rounded-[2rem] bg-slate-900 aspect-[4/3] cursor-pointer shadow-xl">
              <img src="https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=800&auto=format&fit=crop&q=60" className="absolute inset-0 w-full h-full object-cover opacity-60 transition-transform duration-700 group-hover:scale-110" alt="Story Reader"/>
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent"/>
              <div className="absolute bottom-6 left-6 right-6 space-y-2">
          ...
          <Link href="/labs/videos">
            <div className="group relative overflow-hidden rounded-[2rem] bg-slate-900 aspect-[4/3] cursor-pointer shadow-xl">
              <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=800&auto=format&fit=crop&q=60" className="absolute inset-0 w-full h-full object-cover opacity-60 transition-transform duration-700 group-hover:scale-110" alt="Native Videos"/>
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent"/>
              <div className="absolute bottom-6 left-6 right-6 space-y-2">
