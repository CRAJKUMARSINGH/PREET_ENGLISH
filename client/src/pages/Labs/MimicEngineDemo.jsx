...
export default function MimicEngineDemo() {
    var _a = useState('beginner'), selectedLevel = _a[0], setSelectedLevel = _a[1];
    var _b = useState(DEMO_PHRASES.beginner[0]), selectedPhrase = _b[0], setSelectedPhrase = _b[1];
    var _c = useState(0), totalScore = _c[0], setTotalScore = _c[1];
    ...
    var handleScoreUpdate = function (score) {
        setTotalScore(function (prev) { return prev + score; });
        setAttempts(function (prev) { return prev + 1; });
    };
    var averageScore = attempts > 0 ? Math.round(totalScore / attempts) : 0;
    return (<div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      {/* Stats Dashboard */}
      {attempts > 0 && (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <Card>
          ...
          <Card>
            <CardContent className="p-4 text-center">
              ...
              <div className="text-2xl font-bold">{Math.max.apply(Math, (attempts > 0 ? [totalScore / attempts] : [0]))}%</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <Tabs value={selectedLevel} onValueChange={handleLevelChange}>
                ...
                <TabsContent value={selectedLevel} className="mt-4">
                  <div className="space-y-2">
                    {DEMO_PHRASES[selectedLevel].map(function (phrase, index) { return (<Button key={index} variant={phrase === selectedPhrase ? "default" : "outline"} className="w-full text-left justify-start h-auto p-3" onClick={function () { return setSelectedPhrase(phrase); }}>
        ...
        <div className="lg:col-span-2">
          <MimicEngine targetPhrase={selectedPhrase} targetLanguage="en-US" difficulty={selectedLevel} onScoreUpdate={handleScoreUpdate}/>
        </div>
      </div>
      ...
      {/* Call to Action */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="text-center mt-16 p-8 bg-gradient-to-r from-green-50 to-blue-50 dark:from-gray-800 dark:to-gray-700 rounded-lg">
