...
import { Brain, CheckCircle, XCircle, RefreshCw, Zap, Trophy, Calendar, Target, Volume2, RotateCcw, Heart, Sparkles, } from "lucide-react";
...
export default function VocabularyReview() {
    var _a = useSRS(), dueCards = _a.dueCards, reviewCard = _a.reviewCard, getStats = _a.getStats;
    var _b = useState(0), currentIndex = _b[0], setCurrentIndex = _b[1];
    var _c = useState(false), showAnswer = _c[0], setShowAnswer = _c[1];
    var _d = useState(false), sessionComplete = _d[0], setSessionComplete = _d[1];
    var stats = getStats();
    var currentCard = dueCards[currentIndex];
    var handleReview = function (quality) {
        if (!currentCard)
            return;
    ...
    return (<Layout>
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                {/* Session Progress */}
                {dueCards.length > 0 && !sessionComplete && (<div className="space-y-2">
                ...
                {/* Review Card */}
                {dueCards.length > 0 && !sessionComplete ? (<Card className="border-2">
                        <CardContent className="p-8 min-h-[300px] flex flex-col">
                            {/* Word Display */}
                            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6">
                                <div>
                                    <h2 className="text-4xl md:text-5xl font-bold mb-4">
                                        {currentCard === null || currentCard === void 0 ? void 0 : currentCard.word}
                                    </h2>
                                ...
                                {showAnswer && currentCard && (<div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="pt-6 border-t">
                                {!showAnswer ? (<Button onClick={function () { return setShowAnswer(true); }} className="w-full h-14 text-lg font-bold">
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            {qualityButtons.map(function (btn) { return (<Button key={btn.quality} onClick={function () { return handleReview(btn.quality); }} className={cn("h-16 flex-col gap-1", btn.color)}>
