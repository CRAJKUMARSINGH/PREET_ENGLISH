...
export default function Leaderboard() {
    var _a = useLeaderboard(), leaderboard = _a.data, isLoading = _a.isLoading;
    ...
    var top3 = (leaderboard === null || leaderboard === void 0 ? void 0 : leaderboard.slice(0, 3)) || [];
    var rest = (leaderboard === null || leaderboard === void 0 ? void 0 : leaderboard.slice(3)) || [];
    return (<Layout>
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="flex justify-center items-end gap-4 min-h-[220px]">
                    ...
                    {top3[1] && (<div className="flex flex-col items-center">
                            ...
                            <motion.div className="w-24 bg-slate-300/30 rounded-t-lg h-24 mt-2" initial={{ height: 0 }} animate={{ height: 96 }}/>
                        </div>)}

                    {/* 1st Place */}
                    {top3[0] && (<div className="flex flex-col items-center z-10">
                            ...
                            <motion.div className="w-32 bg-yellow-400/20 rounded-t-lg h-32 mt-2" initial={{ height: 0 }} animate={{ height: 128 }}/>
                        </div>)}
                    ...
                    {top3[2] && (<div className="flex flex-col items-center">
                            ...
                            <motion.div className="w-24 bg-amber-700/20 rounded-t-lg h-16 mt-2" initial={{ height: 0 }} animate={{ height: 64 }}/>
                        </div>)}
                </div>

                {/* The Rest of the List */}
                <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    {rest.map(function (entry, index) { return (<div key={entry.user.id} className={cn("flex items-center p-4 gap-4 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors", entry.user.id === 1 && "bg-primary/5 hover:bg-primary/10" // Highlight current user (mock ID 1)
            )}>
                    ...
                    {rest.length === 0 && (<div className="p-8 text-center text-muted-foreground">
