...
import { Sparkles, Award, Heart, BookOpen, Mic, MessagesSquare, Brain, ArrowRight, Star, Zap, TrendingUp, Users, Globe, Play, CheckCircle, Flame } from "lucide-react";
...
export default function ModernHome() {
    var _a, _b, _c, _d;
    var _e = useLessons(), lessons = _e.data, lessonsLoading = _e.isLoading;
    var _f = useProgress(), progress = _f.data, progressLoading = _f.isLoading;
    ...
    var _g = useState(0), activeFeature = _g[0], setActiveFeature = _g[1];
    useEffect(function () {
    ...
    useEffect(function () {
        var interval = setInterval(function () {
            setActiveFeature(function (prev) { return (prev + 1) % features.length; });
        }, 3000);
        return function () { return clearInterval(interval); };
    }, [features.length]);
    return (<Layout>
      <SEO title="Learn English with Confidence" description="Master English with AI-powered lessons, speaking practice, and personalized learning paths designed for Hindi speakers."/>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-20">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="space-y-8">
              {/* Badge */}
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link href="/lesson/1">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-8 py-4 rounded-2xl bg-gradient-to-r from-emerald-600 to-blue-600 text-white font-semibold shadow-lg shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/30 transition-all flex items-center gap-3 group">
                    <Play className="h-5 w-5"/>
                    Start Learning Now
                    <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform"/>
                  </motion.button>
                </Link>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-8 py-4 rounded-2xl border-2 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 font-semibold hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all flex items-center gap-3">
              ...
              <div className="flex items-center gap-4 pt-4">
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4, 5].map(function (i) { return (<div key={i} className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-blue-500 border-2 border-white dark:border-background flex items-center justify-center text-white text-sm font-bold">
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="relative">
              ...
              <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl shadow-emerald-500/10 border border-emerald-100 dark:border-emerald-900/30 p-8 backdrop-blur-sm">
                ...
                {/* Floating Elements */}
                <motion.div animate={{ y: [-10, 10, -10] }} transition={{ duration: 4, repeat: Infinity }} className="absolute -top-4 -right-4 bg-gradient-to-r from-emerald-500 to-blue-500 text-white p-3 rounded-2xl shadow-lg">
                  <Zap className="h-6 w-6"/>
                </motion.div>

                <motion.div animate={{ y: [10, -10, 10] }} transition={{ duration: 3, repeat: Infinity, delay: 1 }} className="absolute -bottom-4 -left-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white p-3 rounded-2xl shadow-lg">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-3 overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: "".concat(percentage, "%") }} transition={{ duration: 1.5, delay: 0.5 }} className="h-full bg-gradient-to-r from-emerald-500 to-blue-500 rounded-full"/>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <AnimatePresence mode="wait">
                      <motion.div key={activeFeature} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className={"p-4 rounded-2xl bg-gradient-to-r ".concat(features[activeFeature].color, " bg-opacity-10 border border-opacity-20")}>
                        <div className="flex items-center gap-3 mb-2">
                          <div className={"p-2 rounded-lg bg-gradient-to-r ".concat(features[activeFeature].color, " text-white")}>
                    ...
                    <div className="flex justify-center gap-2">
                      {features.map(function (_, index) { return (<button key={index} onClick={function () { return setActiveFeature(index); }} className={"w-2 h-2 rounded-full transition-all ".concat(index === activeFeature
                ? 'bg-emerald-500 w-6'
      ...
      <section className="py-16 bg-slate-50 dark:bg-slate-900/50">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <DailyGoalCard lessonsTarget={3} lessonsCompleted={Math.min(completedCount, 3)} xpTarget={50} xpEarned={Math.min(completedCount * 5, 50)} minutesTarget={15} minutesSpent={completedCount * 3}/>
            ...
            <div className="glass-card p-6 rounded-3xl">
              ...
              <div className="w-full bg-secondary h-2 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 transition-all duration-1000" style={{ width: "".concat(Math.min((completedCount / totalLessons) * 100, 100), "%") }}/>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              {/* Lessons Grid */}
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} className="space-y-6">
                <div className="flex items-center justify-between">
                ...
                {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[1, 2, 3, 4].map(function (i) { return (<div key={i} className="glass-card p-6 rounded-2xl animate-pulse">
                    ...
                    {lessons === null || lessons === void 0 ? void 0 : lessons.slice(0, 6).map(function (lesson) { return (<LessonCard key={lesson.id} lesson={lesson} progress={progress === null || progress === void 0 ? void 0 : progress.find(function (p) { return p.lessonId === lesson.id; })}/>); })}
                  </div>)}
              </motion.div>
            ...
            <div className="space-y-6">
              {/* Achievements */}
              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="glass-panel p-6 rounded-3xl">
            ...
            { name: 'Dedicated Learner', nameHindi: 'समर्पित शिक्षार्थी', description: 'Complete 25 lessons', icon: '🌟', xpReward: 100, unlocked: completedCount >= 25 },
        ].slice(0, 4).map(function (achievement, i) { return (<AchievementBadge key={i} {...achievement}/>); })}
                </div>
              </motion.div>

              {/* Leaderboard */}
              {leaderboard && leaderboard.length > 0 && (<motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }}>
                  <LeaderboardCard entries={leaderboard.slice(0, 5).map(function (row, index) {
                var _a, _b;
                return ({
                    rank: index + 1,
              ...
              {/* Quick Actions */}
              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.4 }} viewport={{ once: true }} className="grid grid-cols-2 gap-3">
                <Link href="/speak">
