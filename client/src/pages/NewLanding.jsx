...
import { ArrowRight, Play, CheckCircle, Star, Users, BookOpen, Mic, Brain, Globe, Award, Sparkles, TrendingUp, Zap } from "lucide-react";
import { SaraswatiMascot } from "@/components/SaraswatiMascot";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
export default function NewLanding() {
    var _a = useState(false), isVideoPlaying = _a[0], setIsVideoPlaying = _a[1];
    var _b = useState(0), currentTestimonial = _b[0], setCurrentTestimonial = _b[1];
    var testimonials = [
        {
            name: "Priya Sharma",
    ...
    useEffect(function () {
        var interval = setInterval(function () {
            setCurrentTestimonial(function (prev) { return (prev + 1) % testimonials.length; });
        }, 5000);
        return function () { return clearInterval(interval); };
    }, []);
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50 dark:from-slate-900 dark:via-slate-800 dark:to-emerald-900">
      {/* Navigation */}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                ...
                <Button size="lg" variant="outline" className="border-emerald-200 text-emerald-700 hover:bg-emerald-50 px-8 py-4 text-lg" onClick={function () { return setIsVideoPlaying(true); }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="relative">
              <div className="relative bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 border border-emerald-100 dark:border-emerald-800">
                ...
                <SaraswatiMascot size="lg" mood="teaching" message="नमस्ते! Let's learn English together! 🌟" showCredit={true}/>
      ...
      <section className="py-16 bg-white dark:bg-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map(function (stat, index) { return (<motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className="text-center">
      ...
      <section id="features" className="py-20 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map(function (feature, index) { return (<motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className="group">
                <Card className="h-full border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:-translate-y-2">
                  <CardContent className="p-6">
                    <div className={"inline-flex items-center justify-center w-12 h-12 bg-gradient-to-r ".concat(feature.color, " text-white rounded-xl mb-4")}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative max-w-4xl mx-auto">
            <AnimatePresence mode="wait">
              <motion.div key={currentTestimonial} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.5 }} className="text-center">
                <Card className="border-0 shadow-xl">
                  <CardContent className="p-8">
                    <div className="text-6xl mb-4">{testimonials[currentTestimonial].image}</div>
                    <div className="flex justify-center mb-4">
                      {__spreadArray([], Array(testimonials[currentTestimonial].rating), true).map(function (_, i) { return (<Star key={i} className="w-5 h-5 text-yellow-400 fill-current"/>); })}
                    </div>
            ...
            <div className="flex justify-center mt-8 space-x-2">
              {testimonials.map(function (_, index) { return (<button key={index} onClick={function () { return setCurrentTestimonial(index); }} className={"w-3 h-3 rounded-full transition-colors ".concat(index === currentTestimonial ? 'bg-emerald-500' : 'bg-slate-300')}/>); })}
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Video Modal */}
      <AnimatePresence>
        {isVideoPlaying && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={function () { return setIsVideoPlaying(false); }}>
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} className="bg-white rounded-2xl p-6 max-w-4xl w-full" onClick={function (e) { return e.stopPropagation(); }}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold">Preet English Demo</h3>
                <Button variant="ghost" onClick={function () { return setIsVideoPlaying(false); }}>
                  <X className="w-6 h-6"/>
                </Button>
              </div>
