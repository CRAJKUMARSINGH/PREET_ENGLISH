...
export default function Profile() {
    var _a, _b, _c, _d;
    var progress = useProgress().data;
    var userStats = useUserStats().data;
    var completedLessons = (progress === null || progress === void 0 ? void 0 : progress.filter(function (p) { return p.completed; })) || [];
    var completedCount = completedLessons.length;
    var joinDate = new Date().toLocaleDateString('hi-IN', { month: 'long', year: 'numeric' });
    var stats = {
        currentStreak: Number((_a = userStats === null || userStats === void 0 ? void 0 : userStats.currentStreak) !== null && _a !== void 0 ? _a : Math.min(completedCount, 7)),
        longestStreak: Number((_b = userStats === null || userStats === void 0 ? void 0 : userStats.longestStreak) !== null && _b !== void 0 ? _b : Math.min(completedCount, 7)),
        xpPoints: Number((_c = userStats === null || userStats === void 0 ? void 0 : userStats.xpPoints) !== null && _c !== void 0 ? _c : completedCount * 100),
        level: Number((_d = userStats === null || userStats === void 0 ? void 0 : userStats.level) !== null && _d !== void 0 ? _d : Math.floor(completedCount / 5) + 1),
    };
    var weeklyGoal = Math.min(completedCount, 5);
    var achievements = [
        ...
        <section className="mb-10">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-xs font-bold text-muted-foreground bg-secondary px-3 py-1 rounded-full">
              {achievements.filter(function (a) { return a.unlocked; }).length} / {achievements.length}
            </span>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {achievements.map(function (achievement, i) { return (<div key={i} className={cn("relative p-4 rounded-2xl text-center transition-all duration-300 border overflow-hidden group", achievement.unlocked
                ...
                {achievement.unlocked && (<div className="absolute inset-0 bg-gradient-to-t from-primary/5 to-transparent pointer-events-none"/>)}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <section className="glass-card rounded-3xl p-6">
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
              {completedLessons.length > 0 ? (completedLessons.slice(0, 5).map(function (item) {
            var _a;
            return (<div key={item.id} className="flex items-center justify-between p-3 rounded-xl bg-secondary/30 hover:bg-secondary/50 transition-colors group">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                        <Lucide.BookOpen className="h-4 w-4 text-primary"/>
                      </div>
                      <span className="font-medium text-sm truncate">{((_a = item.lesson) === null || _a === void 0 ? void 0 : _a.title) || 'Lesson'}</span>
                    </div>
            ...
            <div className="relative z-10">
              ...
              <div className="mb-4">
                ...
                <div className="h-4 bg-secondary rounded-full overflow-hidden shadow-inner">
                  <div className="h-full bg-gradient-to-r from-primary to-green-400 rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(28,231,131,0.5)]" style={{ width: "".concat(Math.min((weeklyGoal / 5) * 100, 100), "%") }}/>
                </div>
              </div>
              ...
              {weeklyGoal >= 5 && (<div className="mt-4 p-3 rounded-xl bg-green-500/10 border border-green-500/20 text-center">
