    ...
    return (
        <Layout>
            <SEO
                ...
                description={story?.description || "Interactive Hindi-English stories."}
                ogType="article"
            />
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <article className="space-y-12">
                    ...
                    <div className="relative group">
                        <img
                            src={story.imageUrl || "https://images.unsplash.com/photo-1544947950-fa07a98d237f?auto=format&fit=crop&q=80&w=1200"}
                            alt={story.title}
                            className="w-full h-64 md:h-96 object-cover rounded-[2rem] shadow-2xl border-4 border-white dark:border-slate-800"
                        />
                    ...
                    <div className="space-y-12 leading-relaxed">
                        ...
                        {story.content.split('\n\n').map((para, idx) => {
                            const hindiPara = story.contentHindi?.split('\n\n')[idx];
                            return (
                                <div key={idx} className="group relative space-y-4 p-8 rounded-3xl hover:bg-white dark:hover:bg-slate-900 transition-colors border-2 border-transparent hover:border-primary/10 shadow-sm hover:shadow-xl">
                                    <div className="flex justify-between items-start gap-4">
                                        ...
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="rounded-full shrink-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity"
                                            onClick={() => speak({ text: para, lang: "en-US" })}
                                        >
                    ...
                    {story.vocabulary && (
                        <section className="space-y-6">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                {(typeof story.vocabulary === 'string' ? JSON.parse(story.vocabulary) : story.vocabulary).map((vocab: any, vIdx: number) => (
                                    <div key={vIdx} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <Button
                                                ...
                                                onClick={() => speak({ text: vocab.word, lang: "en-US" })}
                                            >
                ...
                <section className="bg-primary/5 rounded-[2.5rem] p-8 md:p-12 text-center space-y-6">
                    ...
                    <Button
                        size="lg"
                        className="rounded-2xl h-14 px-10 text-lg font-bold"
                        onClick={() => {
                            setShowRating(true);
                            toast({
                            ...
                            trackEvent("story_complete", { storyId: story?.id, title: story?.title });
                        }}
                    >
                        Finish & Get XP
                    </Button>
                ...
                <RatingDialog
                    isOpen={showRating}
                    onClose={() => setLocation("/hindi-stories")}
                    contentType="STORY"
                    contentId={Number(id)}
