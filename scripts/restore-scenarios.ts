...
async function seedScenarios() {
    ...
    for (const s of SCENARIOS) {
        try {
            ...
            // This simple check might be slow for many, but safe.
            // We will rely on unique constraint or just insert.
            // Drizzle doesn't have a simple onConflictDoNothing for sqlite in all versions depending on driver but we can try insert ignore or catch.
            ...
            await db.insert(scenarios).values({
                ...
                yourRole: s.yourRole,
                yourRoleHindi: s.yourRoleHindi,
                partnerRole: s.partnerRole,
                partnerRoleHindi: s.partnerRoleHindi,
                category: s.category,
