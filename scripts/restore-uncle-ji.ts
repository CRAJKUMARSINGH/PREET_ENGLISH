...
const LESSON_ID = 58710; // New ID for the variant
...
async function seed() {
    console.log("Starting seed for Uncle Ji Lesson Variant (#58710)...");

    try {
        const existingLesson = await db.select().from(lessons).where(eq(lessons.id, LESSON_ID)).get();

        if (existingLesson) {
            console.log("Updating existing Lesson #58710...");
            await db.update(lessons).set({
                title: "Introduction & Greetings (Uncle Ji)",
                hindiTitle: "परिचय और अभिवादन (अंकल जी)",
                description: "Variant: Learn to introduce yourself with cultural nuances.",
                content: JSON.stringify(lessonContent),
                difficulty: "Beginner",
                category: "Basic Communication",
                order: 2 // Set strictly after the main lesson
            }).where(eq(lessons.id, LESSON_ID));
        } else {
            console.log("Creating new Lesson #58710...");
            await db.insert(lessons).values({
                ...
                category: "Basic Communication",
                order: 2,
                slug: "introduction-and-greetings-uncle-ji-58710"
            });
        }
        ...
        console.log("Seed for Lesson #58710 completed successfully! 🎉");
    } catch (error) {
